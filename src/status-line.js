...
// their face.
// WS5: once the buddy tier reaches 1 (src/shadow.js classes), one
// buddy-voiced phrase joins the rotation — the buddy occasionally
...
(function () {
  ...
  const BUDDY_PHRASES = [
    null,                                  // tier 0 — the buddy keeps quiet
    'the buddy is listening with you.',
    'the buddy leans closer to the glass.',
    'the buddy breathes in the room with you.',
  ];
  ...
  function rebuild() {
    ...
    const level = g ? g.presence().buddy : 0;
    if (level > 0 && BUDDY_PHRASES[level]) pool.push(BUDDY_PHRASES[level]);
    if (idx >= pool.length) idx = 0;
  }

  function swap(text, buddyVoice) {
    el.classList.add('fading');
    setTimeout(() => {
      el.textContent = text;
      el.classList.toggle('buddy-voice', !!buddyVoice);
  ...
  function showNext() {
    ...
    const isBuddy = pool.length > PHRASES.length && (idx % pool.length) === pool.length - 1;
    idx++;
    swap(line, isBuddy);
  }
