...
(function () {
  var CARDS = [
    ...
    },
    {
      ...
      body: 'what each room is for, and how to work it. buddy: talk to e-lizabeth; two exchanges, then seal — sealing writes the chat to the desktop as an artifact. the stone: draw a buddy, pick an ink, save it; the stone is the hub everything orbits. garden: settle all seventeen facets of the gem; the counter and the mile-markers at five and eleven track it; drafts keep to the satchel or plant into the bed. sea: name what you carry, weigh it on the dots, release — the water takes it and does not keep it. dreams: record, read the hedged reading, add associations (select words in the reading to pin one to them), keep to the book or plant as a garden seed. games: booths that teach dbt skills; results keep to the satchel. divination: ask one question, draw or cast; the reading answers the situation around the question as often as the sentence. learn and methodology: the workbook and its method; the † marks open the cited source. relation: pick a kept thing, give it a verb, and it binds to the buddy; open any bound row to read the full text, or open it in its room. satchel: the book of kept things; margin notes save with ⌘↵ or when you leave the note. trash: bury to remove, dig to bring back — including anything a new build shelves. abstract: cut pairs apart for flow. themes: repaint the machine, desktop included. cohort: the earlier chat room, kept for continuity.',
      key: 'faq · rooms · how to'
    },
    {
      num: '25', title: 'Keeping, Orbits, Slots',
      body: 'keep to the book means the artifact is copied to the satchel, where it stays until buried. orbiting dots on the desktop are a live index of everything kept across rooms — they are not a second copy, and binding one does not move it. the sea text you released stays readable: open the bound row in relation, or the artifact on the desktop, for the full words. nothing uploads, ever; everything lives in this browser profile on this device, and closing the program keeps it all for next time. three save slots live in settings: mess about, serious work, show someone — switching reloads the machine into a separate room with its own tutorial state. the buddy answers deterministically from fixed rules, not from a mind: speak, it reflects, seal after two exchanges.',
