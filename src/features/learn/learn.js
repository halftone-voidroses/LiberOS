...
(function () {
  var CARDS = [
    {
      ...
      marginal: { date: '12.iv.03', note: 're-filed from the poetry drawer — the exceptions kept escaping it.' },
    ...
    {
      num: '04', title: 'TIPP', slug: 'tipp',
      marginal: { date: 'iii.26', note: 'the floor walks these in order. so does the card. it is not decoration.' },
      body: 'tipp is the distress-tolerance arm of dbt<button type="button" class="learn-fn" data-cite="linehan-1993" aria-label="citation">†</button>, and the only set of skills here that asks nothing of thought. it works on the body while the mind is underwater — which is why the floor at the games room keeps it, and why it is walked in order rather than picked from. <span class="learn-skill">t</span>&hairsp;emperature: cold water on the face, the wrists, the back of the neck; the dive reflex slows the heart. <span class="learn-skill">i</span>&hairsp;ntense exercise: ten minutes of anything vigorous — stairs, a fast walk, pushing a wall — until the chemistry burns down to a size you can carry. <span class="learn-skill">p</span>&hairsp;aced breathing: in for four, out for six; a longer out-breath than in, and the body reads that ratio as: the danger has passed. <span class="learn-skill">p</span>&hairsp;aired relaxation: tense a muscle group on the in-breath, release it on the out, and teach the body that releasing is survivable. nothing here is scored, and nothing here is a substitute for the helplines on card 23. <button type="button" class="learn-cross" data-room="games.html#tipp">walk the quiet floor →</button>',
      key: 'dbt · distress tolerance · the body first · quiet floor'
    },

    {
      num: '05', title: 'The Inner Household',
      body: 'two analysts mapped the household inside. klein drew the inner world as peopled: the people of your life, taken in whole, live on inside as objects with moods and agendas, and the buddy here is read as inner object rather than as metaphor<button type="button" class="learn-fn" data-cite="klein-1932" aria-label="citation">†</button>. winnicott found the door between the worlds: the transitional object — the blanket, the teddy, the first not-me possession — the thing that is neither me nor quite the world, and by being both opens an intermediate zone where playing, keeping, and the first culture happen<button type="button" class="learn-fn" data-cite="winnicott-1953" aria-label="citation">†</button>. the journal lives in that zone. its kept artifacts — the first castings, the gem drafts, the seeds from the garden — are transitional objects in the strict sense: held between inner and outer, worthless to anyone else, load-bearing for the keeper. the workbook is a household because the things in it are kept, and keeping is the oldest intermediate act there is.',
      key: 'klein · winnicott · transitional object · journal'
    },

    {
      num: '06', title: 'Shadow Work',
    ...
    {
      num: '08', title: 'Buddy Amplification',
    ...
    {
      num: '09', title: 'Buddy Work', thesis: true,
    ...
    {
      num: '10', title: 'Liber Vacui Method', thesis: true,
...

    {
      ...
      marginal: { date: '30.vi.25', note: 'tried the different hour. it was 04:40. it worked.' },
      key: 'pataphysics · deviation · small change'
    },

    {
      num: '13', title: 'Synchronicity',
    ...
    {
      num: '15', title: 'Defence Mechanisms',
    ...
    {
      num: '18', title: 'Active Imagination',
    ...
    {
      num: '19', title: 'The Collective Unconscious',
    ...
    {
      num: '22', title: 'Fairy Tales and the Shadow',
    ...
    {
      num: '23', title: 'Hard Nights', slug: 'hard-nights',
  ...
  function buildDrawers() {
    ...
    for (var i = 0; i < CARDS.length; i++) {
      (function (c, idx) {
        ...
        d.dataset.idx = idx;
        if (c.slug) d.dataset.slug = c.slug;
  ...
  // Deep links: other rooms cross-link into the shelf with #slug (or
  // #num) so a booth can file a reader straight at the card it cites.
  function indexFromHash() {
    var h = (location.hash || '').replace(/^#/, '').toLowerCase();
    if (!h) return -1;
    for (var i = 0; i < CARDS.length; i++) {
      if (CARDS[i].slug === h) return i;
      if (CARDS[i].num === h) return i;
    }
    return -1;
  }
  ...
  function showCard(idx) {
    ...
    cardEl.innerHTML = ''
      ...
      + (c.marginal
          ? '  <aside class="learn-marginal" aria-label="margin note">'
            + '<span class="learn-marginal-date">' + c.marginal.date + '</span> '
            + c.marginal.note
            + '</aside>'
          : '')
  ...
  function hijackOpen() {
    ...
    // the tour holds the floor: the drawers wait, inert until it ends
    // (at 439 the tour box parks over the stack — modal is the design)
    if (drawersEl) drawersEl.setAttribute('inert', '');
    hijackShow(0);
  }
  function hijackClose() {
    ...
    if (drawersEl) drawersEl.removeAttribute('inert');
    hijackRemember();
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (deep >= 0) showCard(deep);

    // Cross-links out to the room that owns the skill (the Quiet Floor).
    if (cardEl) cardEl.addEventListener('click', function (e) {
      var x = e.target.closest ? e.target.closest('.learn-cross') : null;
      if (x && x.dataset.room) location.href = x.dataset.room;
    });
