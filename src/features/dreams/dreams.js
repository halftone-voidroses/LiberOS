...
(function (global) {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    var raisonClose = el('dreams-raison-close');
    var raisonEl = el('dreams-raison');
    if (helpBtn) helpBtn.addEventListener('click', openRaison);
    if (raisonClose) raisonClose.addEventListener('click', closeRaison);
    if (raisonEl) raisonEl.addEventListener('click', function (e) { if (e.target === raisonEl) closeRaison(); });

    document.addEventListener('keydown', function (e) {
      if (e.key === 'Escape') { closeCite(); closeRaison(); }
    });
    ...
    if (state()) state().on('change', function () {
      ...
      if (currentId) {
        ...
        else { renderAssoc(); syncKeepLabel(currentId); }
      }
    });
  });
