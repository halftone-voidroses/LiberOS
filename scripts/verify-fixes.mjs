...
for (const p of pages) {
  ...
  const found = await page.evaluate(() => {
    const bodies = document.querySelectorAll('[class*="riason-body"], [class*="riason"] .riason-body');
    const hits = [];
...
check('no dev-spec text in any riason panel', devText.length === 0, devText.join(' ;; ') || 'clean');
